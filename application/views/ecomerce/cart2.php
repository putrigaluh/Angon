<html>
<head>
	<title>Cart</title>
</head>
<body>
	<h1>Informasi Keranjang Belanja</h1>
	<?php echo anchor(base_url().'ecomerce/produk', '<< Continue Shopping'); ?>
	<br>
	<br>
	<?php echo form_open(base_url().'ecomerce/shoppingcart/update'); ?>
<table cellpadding="6" cellspacing="1" style="width:60%" border="1">

<tr>
	<th align="center">Delete</th>
  <th>Jumlah</th>
  <th>Nama Produk</th>
  <th style="text-align:right">Harga Produk</th>
  <th style="text-align:right">Sub Total</th>
</tr>

<?php $i = 1; ?>

<?php foreach ($this->cart->contents() as $items): ?>

	<?php echo form_hidden($i.'[rowid]', $items['rowid']); ?>

	<tr>
		<td><?php echo anchor (base_url().'ecomerce/shoppingcart/delete/'.$items['rowid'], 'x'); ?></td>
	  <td><?php echo form_input(array('name' => 'qty'.$i, 'value' => $items['qty'], 'maxlength' => '3', 'size' => '5')); ?></td>
	  <td>
		<?php echo $items['name']; ?>

			<?php if ($this->cart->has_options($items['rowid']) == TRUE): ?>

				<p>
					<?php foreach ($this->cart->product_options($items['rowid']) as $option_name => $option_value): ?>

						<strong><?php echo $option_name; ?>:</strong> <?php echo $option_value; ?><br />

					<?php endforeach; ?>
				</p>

			<?php endif; ?>

	  </td>
	  <td style="text-align:right"><?php echo $this->cart->format_number($items['price']); ?></td>
	  <td style="text-align:right">$<?php echo $this->cart->format_number($items['subtotal']); ?></td>
	</tr>

<?php $i++; ?>

<?php endforeach; ?>

<tr>
  <td colspan="3"> </td>
  <td class="right"><strong>Total</strong></td>
  <td class="right">$<?php echo $this->cart->format_number($this->cart->total()); ?></td>
</tr>

</table>

<p> <?php echo form_submit('', 'Update Keranjang Belanja'); ?></p>
<?php form_close(); ?>
<?php echo anchor(base_url().'ecomerce/checkout', 'Konfirmasi Pembayaran'); ?>
</body>
</html>